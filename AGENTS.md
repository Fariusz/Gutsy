...
- MVP features: Supabase auth + RLS, create/view logs with photo + normalized ingredients, `normalizeIngredients` deterministic function (optional LLM fallback), `get_top_triggers` RPC, Zod-validated endpoints, unit tests + one E2E smoke.
- Data model highlights: `logs`, `ingredients`, `symptoms`, `log_ingredients`, `log_symptoms`. Seed canonical ingredients and ensure `normalizeIngredients` has deterministic fallback for reproducibility.
